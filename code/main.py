...
if __name__ == "__main__":
    ...
    if bow_and_lda:
        ...
        bow_vectorizer = BoWVectorizer(ngram_range = ngram_range)
    ...
    if analyze_topics:
        ...
        if lda_topics and lda_document_topics is not None:
            ...
            topic_analyzer.plot_topic_keywords(
                ...
                mode="model",
                feature_names=bow_vectorizer.get_feature_names(),
                model_components=lda_topic_extractor.get_components(),
                document_topic_matrix=lda_document_topics,
                title="Top topic keywords extracted by LDA"
            )
            topic_analyzer.plot_topic_keywords(
                filename="./results/lda_frequency_keywords.png",
                mode="frequency",
                feature_names=bow_vectorizer.get_feature_names(),
                document_term_matrix=bow_matrix,
                document_topic_matrix=lda_document_topics,
                title="Most frequent n-grams in documents assigned to dominant LDA topic"
            )
            topic_analyzer.plot_topic_overlap_heatmap(
                topics=lda_topics,
                filename="./results/lda_topic_overlap_heatmap.png",
                title="LDA topic keyword overlap"
            )
        ...
        if nmf_topics and nmf_document_topics is not None:
            ...
            topic_analyzer.plot_topic_keywords(
                ...
                model_components=nmf_topic_extractor.get_components(),
                document_topic_matrix=nmf_document_topics,
                title="Top topic keywords extracted by NMF"
            )
            topic_analyzer.plot_topic_keywords(
                filename="./results/nmf_frequency_keywords.png",
                mode="frequency",
                feature_names=tfidf_vectorizer.get_feature_names(),
                document_term_matrix=tfidf_matrix,
                document_topic_matrix=nmf_document_topics,
                title="Most frequent n-grams in documents assigned to dominant NMF topic"
            )
            topic_analyzer.plot_topic_overlap_heatmap(
                topics=nmf_topics,
                filename="./results/nmf_topic_overlap_heatmap.png",
                title="NMF topic keyword overlap"
            )
